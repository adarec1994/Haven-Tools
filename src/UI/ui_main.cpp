...
void runLoadingTask(AppState* statePtr) {
    ...
    const bool isoMode = !state.isoPath.empty();
    if (isoMode) {
        state.preloadStatus = "Mounting Xbox 360 ISO...";
        // Static instance kept alive across loads; X360::Iso::getCurrent()
        // sees the most recently opened one, which is what ERFFile::open()
        // uses when routing iso:// paths.
        static X360::Iso s_iso;
        s_iso.close();
        if (!s_iso.open(state.isoPath)) {
            state.statusMessage = "Error: not a valid Xbox 360 ISO";
        ...
        for (const auto& vp : s_iso.listErfsAsVirtualPaths()) {
            // Sort into the same buckets as the filesystem path so downstream
            // UI logic (RIM scan, encrypted-ERF filtering, etc.) is unchanged.
            const std::string lower = [&]{ std::string s = vp; std::transform(s.begin(), s.end(), s.begin(), ::tolower); return s; }();
            if (lower.size() < 4) continue;
            const std::string ext = lower.substr(lower.size() - 4);
            if (ext == ".erf" || ext == ".lvl") {
                state.erfFiles.push_back(vp);
            } else if (ext == ".rim") {
                state.rimFiles.push_back(vp);
            } else if (ext == ".arl") {
                state.arlFiles.push_back(vp);
            } else if (ext == ".opf") {
        ...
        if (!state.isPreloading) {
            if (ImGuiFileDialog::Instance()->Display("ChooseLauncher", ImGuiWindowFlags_NoCollapse, ImVec2(700, 450))) {
                if (ImGuiFileDialog::Instance()->IsOk()) {
                    ...
                    std::transform(ext.begin(), ext.end(), ext.begin(), ::tolower);
                    if (ext == ".iso") {
                        // Xbox 360 ISO: store the ISO path as the "selected
