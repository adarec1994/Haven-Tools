...
struct RenderSettings {
    ...
    float armorZone4[3] = {1.0f, 1.0f, 1.0f};
    // Per-zone specular RGB tint colors (TINT_MASK_SPECULAR_R/G/B/A from .tnt).
    float armorSpecZone1[3] = {1.0f, 1.0f, 1.0f};
    float armorSpecZone2[3] = {1.0f, 1.0f, 1.0f};
    float armorSpecZone3[3] = {1.0f, 1.0f, 1.0f};
    float armorSpecZone4[3] = {1.0f, 1.0f, 1.0f};
    // Per-zone opacities: how strongly each zone's tint applies.
    // [0..3] map to zones 1..4. Diffuse from TINT_MASK_DIFFUSE_OPACITY,
    // specular from TINT_MASK_SPECULAR_OPACITY.
    float armorDiffOpacity[4] = {1.0f, 1.0f, 1.0f, 1.0f};
    float armorSpecOpacity[4] = {1.0f, 1.0f, 1.0f, 1.0f};
    // Set to 1.0 when an armor TNT preset is loaded (use additive game-math).
    // 0.0 (default) keeps multiplicative chain for manual slider editing.
    float armorTintReplaceFlag = 0.0f;
...
struct AppState {
    ...
    struct CharacterDesigner {
        ...
        float armorTintZone4[3] = {1.0f, 1.0f, 1.0f};
        // Specular RGB per zone (separate from diffuse).
        float armorTintSpecZone1[3] = {1.0f, 1.0f, 1.0f};
        float armorTintSpecZone2[3] = {1.0f, 1.0f, 1.0f};
        float armorTintSpecZone3[3] = {1.0f, 1.0f, 1.0f};
        float armorTintSpecZone4[3] = {1.0f, 1.0f, 1.0f};
        // Per-zone opacities controlling how much of each zone's tint applies.
        float armorTintDiffOpacity[4] = {1.0f, 1.0f, 1.0f, 1.0f};
        float armorTintSpecOpacity[4] = {1.0f, 1.0f, 1.0f, 1.0f};
        // Currently selected TNT preset name (key into AppState::tintCache).
        std::string armorTintPreset;
