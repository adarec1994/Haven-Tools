    ...
    for (int pass = 0; pass < 2; pass++) {
        ...
        for (const auto& draw : s_levelDraws) {
            ...
            if (draw.materialIndex != lastMatIdx || curAlpha != lastAlpha || isSelected || wasSelected) {
                ...
                // No per-character tint zones in this code path — set all four to
                // identity (white) so the shader's `lerp(c, c * zone.rgb, mask)`
                // becomes a no-op even when the tint mask alpha is non-zero.
                perMat.tintZone1[0] = perMat.tintZone1[1] = perMat.tintZone1[2] = 1.0f;
                perMat.tintZone2[0] = perMat.tintZone2[1] = perMat.tintZone2[2] = 1.0f;
                perMat.tintZone3[0] = perMat.tintZone3[1] = perMat.tintZone3[2] = 1.0f;
                perMat.tintZone4[0] = perMat.tintZone4[1] = perMat.tintZone4[2] = 1.0f;
                perMat.tintSpecZone1[0] = perMat.tintSpecZone1[1] = perMat.tintSpecZone1[2] = 1.0f;
                perMat.tintSpecZone2[0] = perMat.tintSpecZone2[1] = perMat.tintSpecZone2[2] = 1.0f;
                perMat.tintSpecZone3[0] = perMat.tintSpecZone3[1] = perMat.tintSpecZone3[2] = 1.0f;
                perMat.tintSpecZone4[0] = perMat.tintSpecZone4[1] = perMat.tintSpecZone4[2] = 1.0f;
                for (int i = 0; i < 4; i++) { perMat.tintDiffOpacity[i] = 1.0f; perMat.tintSpecOpacity[i] = 1.0f; }
    ...
    if (!model.meshes.empty()) {
        ...
        for (int pass = 0; pass < 2; pass++) {
            for (size_t meshIdx = 0; meshIdx < model.meshes.size(); meshIdx++) {
                ...
                // Zone 4 is armor-only; default to white (no-op) so the alpha
                // channel of any non-armor tint map can't accidentally tint anything.
                static const float kZoneIdentity[3]   = {1.0f, 1.0f, 1.0f};
                static const float kOpacityIdentity[4] = {1.0f, 1.0f, 1.0f, 1.0f};
                const float* zone4     = kZoneIdentity;
                const float* specZone1 = kZoneIdentity;
                const float* specZone2 = kZoneIdentity;
                const float* specZone3 = kZoneIdentity;
                const float* specZone4 = kZoneIdentity;
                const float* diffOpacity = kOpacityIdentity;
                const float* specOpacity = kOpacityIdentity;
                float tintReplaceFlag = 0.0f;
                ...
                if (meshNameLower.find("hed") != std::string::npos ||
                    ...
                    zone4 = settings.armorZone4;
                    specZone1 = settings.armorSpecZone1;
                    specZone2 = settings.armorSpecZone2;
                    specZone3 = settings.armorSpecZone3;
                    specZone4 = settings.armorSpecZone4;
                    diffOpacity = settings.armorDiffOpacity;
                    specOpacity = settings.armorSpecOpacity;
                    tintReplaceFlag = settings.armorTintReplaceFlag;
                ...
                memcpy(perMat.tintZone4, zone4, 12); perMat.tintZone4[3] = 0;
                memcpy(perMat.tintSpecZone1, specZone1, 12); perMat.tintSpecZone1[3] = 0;
                memcpy(perMat.tintSpecZone2, specZone2, 12); perMat.tintSpecZone2[3] = 0;
                memcpy(perMat.tintSpecZone3, specZone3, 12); perMat.tintSpecZone3[3] = 0;
                memcpy(perMat.tintSpecZone4, specZone4, 12); perMat.tintSpecZone4[3] = 0;
                memcpy(perMat.tintDiffOpacity, diffOpacity, 16);
                memcpy(perMat.tintSpecOpacity, specOpacity, 16);
                perMat.tintReplaceFlag[0] = tintReplaceFlag;
                perMat.tintReplaceFlag[1] = 0.0f;
                perMat.tintReplaceFlag[2] = 0.0f;
                perMat.tintReplaceFlag[3] = 0.0f;
